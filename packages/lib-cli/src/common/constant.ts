...
function findRootDir(dir: string): string {
  if (existsSync(join(dir, 'lib.config.js'))) {
    return dir
  }
...
export const LIB_CONFIG_FILE = join(ROOT, 'lib.config.js')
...
export function getLibConfig() {
  delete require.cache[LIB_CONFIG_FILE]

  try {
    return require(LIB_CONFIG_FILE)
  } catch (err) {
    return {}
  }
}
...
function getSrcDir() {
  ...
  const srcDir = get(libConfig, 'build.srcDir')
